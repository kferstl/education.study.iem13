...
namespace Examplinvi
{
    ...
    class Program
    {
        ...
        static  void Main()
        {
			...
			TwitterCredentials.CreateCredentials("296364810-94ju4YAFSjVUV29JayjKwqIkJwHjJKaw8d4mjmrj", "nXQuvGr9m0BzuJFoSj1LGKxKdup9hK8IsNYYzkAz9rjet", "eiHvFURg52Yomn0xEIBPJ6inL", "areKDP4TrO3CWijEgIbzytJ2GKn4Ax8VpgRwUfLAI3tb8pCPiA");

			var datumBox = new DatumboxAPI.DatumboxAPI("a76853008d862cdc98cb1c5bc6973f57");
			...
			stream.AddTrack("russia");
			stream.AddTrack("krim");
			//stream.AddTrack("spain");

			stream.MatchingTweetReceived += (sender, args) => {
				if (args != null && args.Tweet != null && args.Tweet.Text != null) {
					Task.Run(() => {
						var sentiment = datumBox.TwitterSentimentAnalysis(args.Tweet.Text);

						/*
						var scoreDataMap = new Dictionary<string, string>();
						scoreDataMap.Add("text", args.Tweet.Text);
						var sentiment = RepustateClient.getSentiment(RepustateClient.FORMAT_JSON, scoreDataMap);
						*/
						...
						lock(datumBox){
							...
							Console.WriteLine("Positive: {0}  Sum: {1} | {2}%  {3}", pos, sum, Math.Round((pos / sum) * 100.0, 2), sentiment);
						}
